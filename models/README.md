...
模型选择优先级:**已下载 > 配置指定 > 默认 v1**

| 场景 | 结果 |
|---|---|
| `models/icon_detect_v2.onnx` 已存在 | ✅ 自动用 v2(无需任何配置,不浪费 80MB) |
| v2 未下载 + 配置 `icon_model: v2` | 自动下载 v2 → 下载失败回退 v1 |
| v2 未下载 + 未配置(默认) | 用 v1(仓库自带,零下载) |

**含义**:用户下载了 v2 → 一直用 v2,不会被默认配置回滚;
         想回到小模型 → 删除 `models/icon_detect_v2.onnx` 即可。
