# 全局 AI 助手(独立产品)实施规划 v3

> **For Hermes:** 执行时按本规划任务逐项实施,每任务 TDD + 验证。

**Goal:** 构建一个**独立分发**的桌面 AI 助手产品 — 任何人下载/安装、配置 API key 即可使用。常驻托盘,热键/语音/点击唤起对话窗口;理解屏幕与用户当前操作;简单操作快速响应,复杂任务深度思考;执行键鼠操作与系统命令;变更类操作需用户确认。

**Architecture:** 完全自建的 Python 桌面应用,不依赖 Hermes 运行时。核心闭环:输入(热键/语音/对话窗)→ 上下文采集(前台窗口/截图)→ 复杂度分流(快/深)→ LLM 推理(mimo-v2.5)→ 工具执行(确认门)→ TTS/文字反馈。窗口控制复用本仓库 `window_control` 内核(独立库,无 Hermes 依赖)。

**Tech Stack:** Python 3.11、Tkinter(GUI)、pywin32、Pillow、openai SDK(mimo-v2.5 默认)、faster-whisper、sounddevice、pystray、CosyVoice/edge-tts、PyInstaller(打包)。

---

## 0. 产品定位(用户已确认)

- **独立产品**:不依赖 Hermes,任何用户下载安装 + 配 API key 即用
- **目标用户**:普通电脑用户(非开发者),操作门槛低
- **差异化**:桌面键鼠控制 — 市面主流 Harness(Claude Code/ChatGPT 等)都不做的稀缺能力
- **默认模型**:mimo-v2.5(文本+视觉统一),API key 用户自备,首次运行引导配置
- **项目根**:`D:\data\opencode_temp_code\window_control_core`(window_control 即内核模块)
- **虚拟环境**:项目内 `.venv`;打包后为独立 exe(内嵌 venv)
...
- TTS:CosyVoice 优先,edge-tts 兜底(无 key 也能用),异步播报
...
### 1.6 首次运行向导(setup_wizard)
- 首次启动检测无 key → 弹出配置窗:模型提供商/API key/热键/语音开关
- 配置存 `~/.global_assistant/config.json`(不进代码仓库)
- 内置"模型连通性测试"按钮

### 1.7 打包分发(PyInstaller)
- `assistant.spec`:单目录模式,内嵌 venv 依赖;输出 `全局AI助手.exe`
- 用户侧零 Python 依赖;faster-whisper 模型首次使用时提示下载(或内置 base 模型)
- 分发形态:zip 免安装版(首选)/ NSIS 安装包(二期)
...
- `.venv` 安装: `pywin32 pillow openai python-dotenv faster-whisper sounddevice pystray requests edge-tts dashscope`
...
- `classify_complexity()` → fast/deep;`Agent.run_fast()` 单轮直答;`Agent.run_deep()` function calling 循环
- 系统提示词(中文):独立桌面助手身份、三模式、安全规则
- 测试:mock executor 验证循环;`classify_complexity("打开计算器")=="fast"`、`"规划一个自动化脚本"=="deep"`

### Task 9: TTS / STT
**Files:** Create `voice/tts.py` + `voice/stt.py`
- `speak(text)`:CosyVoice(key 可无 → edge-tts 免费兜底)→ 播放;异步
...
- 验证: `dist/全局AI助手.exe` 在**无 Python 环境**的机器可运行(或本机另一用户账户)
...
2. **faster-whisper 模型体积**:base ~150MB;打包默认不内置,首次用提示下载(或提供内置版构建)
3. **Tkinter 观感**:轻量但朴素;产品化后可换 PyQt(二期美化)
4. **常驻麦克风隐私**:唤醒词二期默认关;一期仅热键后录音,UI 明示录音状态
5. **PyInstaller 体积**:单目录 ~300MB(含 whisper);可拆成"核心包 + 语音包"
6. **API key 安全**:存用户目录 config.json(非仓库);文档明示 key 归属用户自己

## 5. 已确认事项清单

- [x] 独立产品,不依赖 Hermes,任何人可安装使用
- [x] 项目根 = `window_control_core`,window_control 为内核
- [x] 项目内 `.venv` 开发,打包为独立 exe
- [x] 默认模型 mimo-v2.5,用户自备 API key,首次运行向导配置
- [x] 双速推理(简单快/复杂深)
- [x] 对话窗口(文字 + 本地文件选择)
- [x] 语音监听(STT)+ TTS 反馈
